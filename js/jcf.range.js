	...
	jcf.addModule(function($) {
		return {
			...
			initStructure: function() {
				...
				this.values = this.isMultiple ? this.realElement.attr('value')
													.split(',') : [this.realElement.val()];
				this.handleCount = this.isMultiple ? this.values.length : 1;

				// create range display
				this.rangeDisplayWrapper = $(this.options.rangeDisplayWrapper)
				...
				while (this.createdHandleCount < this.handleCount) {
					...
					if (this.createdHandleCount > 1) {
						...
						this.rangeMid = this.rangeMid.add($(this.options.rangeDisplay)
			...
			onTrackPress: function(e) {
				...
				if (!this.realElement.is(':disabled') && !this.activeDragHandle) {
					...
					this.handles.removeClass(this.options.activeHandleClass)
			...
			updateRanges: function() {
				...
				if (this.rangeMin) {
					...
					this.rangeMin.css(this.offsetProperty, 0)
						.css(this.sizeProperty, handle.style[this.offsetProperty]);
				}
				if (this.rangeMax) {
					handle = this.handles[this.handles.length - 1];
					this.rangeMax.css(this.offsetProperty, handle.style[this.offsetProperty])
						.css(this.sizeProperty, 100 - parseFloat(handle.style[this.offsetProperty]) + '%');
				}
				if (this.rangeMid) {
